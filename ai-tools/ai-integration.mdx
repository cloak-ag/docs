...
7) Use SDK defaults for program and circuits. The relay has no default: pass `relayUrl` on every `transact` / `fullWithdraw` call, sourced from the `CLOAK_RELAY_URL` env var. Do not ask end users to configure program or circuits.
...
9) Use SDK defaults for program, mint, and circuits (`CLOAK_PROGRAM_ID`, `NATIVE_SOL_MINT`, `CIRCUITS_BASE_URL` from `cloak_sdk::constants`). Do not expose those as user config. There is no relay constant in `cloak_sdk::constants`, so set `opts.relay_url` in code to the mainnet relay URL as shown in item 5, and do not expose it as user config either.
...
- no user-facing circuits/program config; set `relayUrl: "https://api.cloak.ag"` in the transact options, since the SDK has no relay default and `transact()` otherwise throws `Viewing key registration is mandatory: relayUrl is required.` `CLOAK_RELAY_URL`, when set, overrides it.
...
- env vars: `SOLANA_RPC_URL`, `KEYPAIR_PATH`
- no user-facing circuits/program config; set `opts.relay_url` to `https://api.cloak.ag`, as in requirement 5 of the Rust one-shot prompt above
