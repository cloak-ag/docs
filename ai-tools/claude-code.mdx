...
**CRITICAL: Devnet requires explicit programId** (SDK defaults to mainnet):
...
## Error Handling
CloakError has category (wallet|network|prover|relay|validation|environment) 
...
```typescript
{
  category: "wallet" | "network" | "prover" | "relay" | "validation" | "environment";
  retryable: boolean;
  message: string;
}
```
...
- `0x1001` - RootNotFound: Merkle root expired (SDK auto-retries)
...
Use `isRootNotFoundError(error)` to check for stale root errors.
