...
The mainnet SDK ships the production program ID and the 0.2.0 circuits paths as defaults. The submission endpoint is not defaulted: pass `relayUrl` in your options, or set `CLOAK_RELAY_URL` in the environment. Omitting it does not fail cleanly. `swapWithChange` throws `relayUrl is required for swapUtxo`, and a shielded transfer that also passes `depositorKeypair` falls back to submitting under your own wallet, which pays the fee and links the transaction to you. Also swap `DEVNET_MOCK_USDC_MINT` for Circle USDC (`EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v`) and use a mainnet RPC for your `Connection`. Cloak's mainnet `swapWithChange` routes through Jupiter (real liquidity); devnet routes through the Pyth-priced mock path.
