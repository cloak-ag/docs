...
Getting in is free, and staying in is free. Leaving costs 0.3% plus a small fixed fee: 0.005 SOL when what's leaving is SOL, and 0.45 USDC or USDT when it's a stablecoin.
...
| [Bridge in](/guide/bridge) from another chain | **Free** — arriving carries no protocol fee | The route charges its own cut on the origin side; Solana network costs come out of the bridged amount, so your wallet pays nothing |
...
| Private send to an address — USDC/USDT | **0.45 USDC/USDT fixed + 0.3%** | Submitted for you; no gas from your wallet |
...
**Only the program charges you.** Fees are set per pool in the program's on-chain configuration and collected on-chain by the Cloak program into a treasury account. Nothing else in the system charges you — no service margin, no spread, no hidden markup.

**Bridged funds arrive free of protocol fees, and leave on the standard terms.** A bridge deposit is a deposit, so nothing is charged to arrive. Two costs sit outside Cloak's fee table and are quoted to you before you send: the route's own cut on the origin side, and the Solana network cost of getting the tokens into the pool, which comes out of the bridged amount because you arrive on Solana holding no SOL. Details in [Bridging in](/guide/bridge#what-it-costs).
...
<Tip>
  Other Solana privacy protocols charge a flat 1% on every withdrawal. Cloak charges 0.3% plus a fixed fee of 0.005 SOL for SOL, or 0.45 USDC/USDT for stablecoins.
</Tip>
...
**Send 100 USDC.** The recipient receives exactly 100 USDC. The fee is 0.45 USDC fixed + 0.30 USDC (0.3% of 100 USDC), so your private balance decreases by 100.75 USDC.
