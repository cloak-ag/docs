...
description: "The three concepts behind Cloak custody — private balance, notes, and the backup file that recovers them"
icon: "file-shield"
---
...
<CardGroup cols={3}>
  <Card title="Private balance" icon="wallet">
    Your shielded funds. It lives only in your browser, encrypted locally — Cloak's servers never hold it, and there is no cloud sync.
  </Card>
  <Card title="Notes" icon="note-sticky">
    The units inside your private balance. Each shielded deposit and each payment you receive becomes a note.
  </Card>
...
Under the hood, notes are commitments in an on-chain Merkle tree, spendable only with secrets held on your device — and that is all the cryptography this page needs.
...
It is also a point-in-time snapshot. Notes created after you exported it — new deposits, payments received — are not in the old file. Re-export after new activity.
...
**Losing your wallet does not lose your funds.** If your browser data or your backup file survives, your shielded funds survive too: any connected wallet can spend restored notes. What goes with the old wallet is its [viewing key](/guide/compliance) — read-only access to the compliance history of your old activity, not the funds themselves.
...
On import, Cloak checks every note against the chain and prunes anything already spent, so an older file restores cleanly — you get exactly the notes that are still unspent. Then connect a wallet and use the balance as normal.
