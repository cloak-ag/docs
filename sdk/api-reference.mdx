...
Custom indexer URL. Defaults to `https://api.cloak.ag`.
</ParamField>
...
Scan blockchain for notes belonging to this wallet (requires `cloakKeys`).

```typescript
async scanNotes(options?: ScanNotesOptions): Promise<ScannedNote[]>
```

<ParamField body="options" type="ScanNotesOptions">
<Expandable title="Properties">
  <ParamField body="startIndex" type="number" default="0">
  First leaf index to scan.
  </ParamField>
  
  <ParamField body="endIndex" type="number">
  Last leaf index (defaults to latest).
  </ParamField>
  
  <ParamField body="batchSize" type="number" default="100">
  Notes per batch.
  </ParamField>
...
const sdk = new CloakSDK({
  keypairBytes,
  cloakKeys: generateCloakKeys(),
  network: "devnet",
});

const notes = await sdk.scanNotes({
  onProgress: (i, n) => console.log(`Scanning ${i}/${n}`),
});
...
Recover a deposit that succeeded on-chain but failed to register.
...
const result = await sdk.recoverDeposit({
  ...
  note: savedNote, // Optional if you have it
});

if (result.success) {
  console.log("Recovered! Leaf:", result.leafIndex);
}
```
