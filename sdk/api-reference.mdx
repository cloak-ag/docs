...
Custom program ID. **Required for devnet** - SDK defaults to mainnet.
</ParamField>
...
The SDK computes Merkle proofs directly from on-chain state without an indexer:
...
Thrown when the Merkle root used in a proof is no longer in on-chain history.

```typescript
import { RootNotFoundError, isRootNotFoundError } from "@cloak.ag/sdk";

try {
  await sdk.withdraw(connection, note, recipient);
} catch (error) {
  if (isRootNotFoundError(error)) {
    // The SDK automatically retries with fresh proof (up to 3 times)
    // This error only surfaces after all retries are exhausted
    console.log("Root expired after max retries");
  }
}
```
...
Map of on-chain program error codes to messages. See [Error Handling](/sdk/error-handling) for the complete reference.

```typescript
import { ShieldPoolErrors } from "@cloak.ag/sdk";

// Common error codes:
// 0x1001 = RootNotFound (stale proof)
// 0x1010 = ProofInvalid
// 0x1020 = DoubleSpend
// 0x1076 = ProofVerificationFailed
