...
Cloak uses a **deposit-withdraw** pattern similar to tornado.cash but optimized for Solana:
...
The SDK computes proofs automatically from on-chain state during withdrawal:

```typescript
// SDK handles this internally, but you can also compute manually
...
### Using keys with the SDK

Initialize the SDK with Cloak keys for advanced features:

```typescript
const sdk = new CloakSDK({
  keypairBytes: keypair.secretKey,
  cloakKeys: keys, // Optional: for encrypted note features
  network: "devnet",
});

// Generate notes using wallet keys
const note = await sdk.generateNote(100_000_000, true); // useWalletKeys = true
```
...
|-----|-------------|---------|
| Fixed | 0.003 SOL | Relay gas costs |
| Variable | 0.5% of amount | Protocol revenue |
