...
Fees apply only when value leaves the pool (`externalAmount < 0`); deposits and
shielded→shielded transfers are fee-free.

For SOL withdrawals/swaps:

- `gross` = absolute public withdrawal amount
- `fee` = `5_000_000` lamports fixed + `gross * 3 / 1000`
- `net` = `gross - fee`

For SPL (USDC/USDT) withdrawals:

- `fee` = `450_000 + floor(gross * 3 / 1000)` in token base units (0.45 token fixed plus 0.3%), charged in the withdrawn token

Minimum deposit: `10_000_000` lamports (`0.01 SOL`) for the SOL pool, `1_000_000` base units (`1.00 token`) for USDC/USDT. See [Fee Model](/protocol/fee-model).
