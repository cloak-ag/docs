...
try {
  ...
  if (error instanceof CloakError && error.category === "relay") {
    if (error.message.includes("RootNotFound")) {
      ...
      showMessage("Merkle root not found. Check your configuration.");
...
This is the most common error on devnet. It means the Merkle root in your proof doesn't match any valid root in the on-chain root history.

**Most common cause: Missing `programId` configuration**

The SDK defaults to the mainnet program ID. When using devnet, you **must** explicitly set the devnet program ID:

```typescript
import { PublicKey } from "@solana/web3.js";

// ❌ WRONG - Will use mainnet program, causing RootNotFound on devnet
const sdk = new CloakSDK({
  keypairBytes: keypair.secretKey,
  network: "devnet",
});

// ✅ CORRECT - Explicitly set devnet program ID
...
**Other causes:**
- The root was pushed out of history (100 roots retained). Try withdrawing sooner after deposit.
- Deposit and withdrawal are on different networks (e.g., deposited on mainnet, trying to withdraw on devnet)

<Warning>
If you deposited with the wrong program ID, those funds are in a different Merkle tree. You'll need to withdraw using the same program ID used during deposit.
</Warning>
