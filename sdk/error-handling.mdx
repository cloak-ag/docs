...
type ErrorCategory = 
  ...
  | "indexer"      // Indexer service issues
...
Indexer errors affect deposit registration and proof fetching:

```typescript
try {
  await sdk.deposit(connection, amount);
} catch (error) {
  if (error instanceof CloakError && error.category === "indexer") {
    if (error.message.includes("duplicate")) {
      // Note already registered - this is recoverable
      showMessage("Deposit already recorded");
    } else if (error.retryable) {
      showMessage("Indexer sync in progress. Retrying...");
    }
  }
}
```
...
export async function handleCloakError(
  ...
  switch (error.category) {
    ...
    case "indexer":
      if (error.retryable && onRetry) {
        showMessage("Indexer syncing. Retrying...");
        await delay(2000);
        await onRetry();
        return true;
      }
      showMessage("Indexer error: " + error.message);
      return false;
...
If a deposit transaction succeeds but registration fails (browser crash, network issue), use the recovery service:

```typescript
import { CloakSDK } from "@cloak.ag/sdk";

// You saved the note and signature before the crash
const savedNote = JSON.parse(localStorage.getItem("pending_note")!);
const signature = localStorage.getItem("pending_signature")!;

const sdk = new CloakSDK({ keypairBytes, network: "devnet" });

const recovery = await sdk.recoverDeposit({
  signature,
  commitment: savedNote.commitment,
  note: savedNote,
  onProgress: (status) => console.log("Recovery:", status),
});

if (recovery.success) {
  console.log("Recovered! Leaf index:", recovery.leafIndex);
  
  // Update the note with recovered data
  savedNote.leafIndex = recovery.leafIndex;
  savedNote.root = recovery.root;
  savedNote.merkleProof = recovery.merkleProof;
