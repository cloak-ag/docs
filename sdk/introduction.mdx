...
| Relay URL | No default. Pass `relayUrl` or set `CLOAK_RELAY_URL`; on mainnet the value is `https://api.cloak.ag` |
...
`relayUrl` is never defaulted. Pass it on every `transact` call, including shield, send and withdraw, and on the swap helpers. On Node you can instead set `CLOAK_RELAY_URL` in the process environment; in a browser bundle pass the option explicitly. Omitting it raises an error rather than falling back to any deployment. For mainnet the value is `https://api.cloak.ag`.
