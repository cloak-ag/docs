...
<Warning>
**Important:** On devnet, you must explicitly set the `programId` to the devnet program address. The SDK defaults to mainnet, which will cause `RootNotFound` errors if used on devnet.
</Warning>

```typescript
// Devnet program ID (required for devnet)
const DEVNET_PROGRAM_ID = new PublicKey("3EvH6XYQir7D2RyzCdP7QFmJFfSPfdeoB8VnpLdEF4Kr");

// Mainnet program ID (default)
const MAINNET_PROGRAM_ID = new PublicKey("c1oak6tetxYnNfvXKFkpn1d98FxtK7B68vBQLYQpWKp");
```
...
// Devnet program ID - REQUIRED for devnet usage
const DEVNET_PROGRAM_ID = new PublicKey("3EvH6XYQir7D2RyzCdP7QFmJFfSPfdeoB8VnpLdEF4Kr");
...
async function main() {
  ...
  // 2. Initialize the SDK with devnet program ID
  ...
  console.log("\n🔐 Note commitment:", note.commitment.slice(0, 16) + "...");
  
  // 5. Withdraw to recipient
  console.log("\n📤 Withdrawing to", recipient.toBase58().slice(0, 8) + "...");
...
<Warning>
Always set `programId` when using devnet. Without it, the SDK uses mainnet addresses, causing proof verification failures.
</Warning>
</Step>
