...
with `serializeUtxo`, read back with `deserializeUtxo`. `fullWithdraw` is the single exemption,
because it leaves nothing shielded. Full treatment:
...
try {
  // fullWithdraw is used here because it is the one flow with no change note to
  // bank. Every other flow on this page returns notes you must persist first.
