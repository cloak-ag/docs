...
5. **Proof generation**: ark-circom + ark-groth16 emitted a 256-byte proof. Artifacts (wasm + zkey) cache under `~/Library/Caches/cloak-sdk/circuits/0.1.0/` on macOS, `~/.cache/cloak-sdk/circuits/0.1.0/` on Linux. That pinned `0.1.0` version is the constraint described in the warning at the top of this page; the directory name is derived from the constant and does not change even after you seed it with the 0.2.0 bundle.
