...
5. **Proof generation**: ark-circom + ark-groth16 emitted a 256-byte proof. Artifacts (wasm + zkey) cache under `~/Library/Caches/cloak-sdk/circuits/<bundle>/` on macOS, `~/.cache/cloak-sdk/circuits/<bundle>/` on Linux. The deployed program accepts only the `0.2.0` ceremony bundle (see [Release notes 2026-08](/releases/2026-08-mainnet)).
