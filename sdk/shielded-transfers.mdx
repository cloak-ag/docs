...
const aliceWallet = Keypair.fromSecretKey(
  Uint8Array.from(JSON.parse(process.env.ALICE_SECRET_KEY!)),
);
const bobWallet = Keypair.fromSecretKey(
...
const deposit = await transact(
  ...
  },
  {
    ...
    walletPublicKey: aliceWallet.publicKey,
...
async function sendToBob(
  ...
  try {
    ...
    // Bank the change before you render "sent". This is its only copy.
