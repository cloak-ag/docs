...
Under the hood, SDK builds proof + `264`-byte public inputs and submits through relay transaction orchestration.
...
`swapWithChange` sends `TransactSwap` payload (`proof + 264-byte public inputs + swap params`) through relay swap orchestration.
...
- Registration signs fixed sign-in message and submits through relay compliance registration.
...
Relay can serve risk quotes; you can also provide your own quote backend/direct instruction callback.
...
- Relay-backed Merkle reconstruction is preferred for older indices.
...
- Protocol behavior: [Shield Pool Program](/protocol/shield-pool.mdx)
- Runtime lifecycle: [Transaction Flows](/platform/transaction-flows.mdx)
- Full SDK exports: [API Reference](/sdk/api-reference.mdx)
